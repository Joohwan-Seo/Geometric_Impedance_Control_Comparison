...
%     Jb = Je_fun(q1,q2,q3,q4,q5,q6);
%     Jb_dot = Je_dot_fun(q1,q2,q3,q4,q5,q6,dq1,dq2,dq3,dq4,dq5,dq6);
...
%     M_tilde = pinv(Jb)' * M * pinv(Jb); 
    ...
    if abs(det(Jb)) < 0.01 %|| (k > 150 && k < 783)
        sing_arr(k) = 1;
        F = - f_g - Kxi * e_xi;
    else
...
disp('Geometric 2 T rms')
disp(T_rms')
disp(F_rms');

disp(norm(T,'fro')); disp(norm(T1,'fro'));

fprintf('Geometric 2 pos error rms %f \n', pos_err_rms);
fprintf('lyap rms %f \n', lyap_rms);
fprintf('err rms %f\n', err_rms);
